...
class EpisodeMapper extends Mapper
{
    ...
    function getEpisodesBy(?PDO $pdo, int $animeId, int $season, string $sort, PlatformMapper $platformMapper, AnimeMapper $animeMapper, CountryMapper $countryMapper, EpisodeTypeMapper $episodeTypeMapper, LangTypeMapper $langTypeMapper): ?array
    {
        if ($sort == 'asc')
            ...
            $request = $pdo->prepare("SELECT * FROM $this->tableName WHERE anime_id = :animeId AND season = :season ORDER BY id_episode_type, number DESC, id_lang_type");
